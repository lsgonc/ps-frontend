  ...
  return (
    <div className="flex gap-2 text-white px-[80px] ">
      {data.map((list:any, index:number) => (
        <div key={index} className="flex flex-col gap-4 border border-[#4E4E4E] pt-[16px] px-[16px] pb-[24px] rounded-md">
          ...
          {tasks.filter((task:any) => task.listId === list.id).map((task:any) => (
            <TaskCard key={task.id} date={task.finishAt <= Date.now() ? "onTime" : "late"} 
                      priority={task.priority} title={task.title} description={task.description} 
                      taskDate={task.finishAt} isFinished={task.finishedAt} id={task.id} listId={list.id} ></TaskCard>
          ))}
